...
router.get('/', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(`${SELECT} WHERE t.done = $1 ${order}`, [done]);
    res.json(rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.post('/', async (req, res) => {
  ...
  try {
    const { rows: [row] } = await pool.query(
      ...
      [description.trim(), notes || null, due_date || null, assigned_to || null, job_id || null, req.user.id]
    );
  ...
  try {
    const { rowCount } = await pool.query(
      ...
      [description?.trim() || null, notes || null, due_date || null,
       assigned_to || null, job_id || null, req.params.id]
    );
